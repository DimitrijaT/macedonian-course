 ...
 */
export const createConnectQuestion = (
  vocab: Vocabulary[],
  lessonId: string
): QuizQuestion | null => {
  ...
  const shuffledVocab = [...uniqueVocab]
 ...
 */
export const createGrammarQuestions = (
  tables: GrammarTable[],
  lessonId: string
): QuizQuestion[] => {
  ...
  tables.forEach((table, tIndex) => {
    ...
    rows.forEach((row, rIndex) => {
      ...
      // e.g. "Jas", "-m", "Gledam"
      if (
        headers.some((h) => h.includes("person")) &&
        (headers.some((h) => h.includes("verb")) ||
          headers.some((h) => h.includes("suffix")) ||
          headers.some((h) => h.includes("full verb")))
      ) {
        const personIdx = headers.findIndex((h) => h.includes("person"));
        // Find the column that has the FULL verb or the main verb form
        const verbIdx = headers.findIndex(
          (h) =>
            h.includes("full verb") ||
            (h.includes("verb") && !h.includes("suffix"))
        );
      ...
      // e.g. Jas, Jadam, Mislam
        ...
        const verbCols = headers
          .map((_h, i) => i)
          .filter((i) => i !== personIdx);
        const randomVerbColIdx =
          verbCols[Math.floor(Math.random() * verbCols.length)];
      ...
      // 3. GENDER RULES (Gender + Example)
        ...
        (headers.includes("example") || headers.includes("word"))
      ) {
        const genderIdx = headers.findIndex((h) => h.includes("gender"));
        const exampleIdx = headers.findIndex(
          (h) => h.includes("example") || h.includes("word")
        );
        ...
        if (genderIdx !== -1 && exampleIdx !== -1) {
          ...
          const finalDistractors = allGenders.filter(
            (g) =>
              !g.toLowerCase().includes(gender.toLowerCase().substring(0, 3))
          );
      ...
      // 5. TRANSLATION (Macedonian + English)
        ...
        (headers.includes("macedonian") || headers.includes("question")) &&
        headers.includes("english")
      ) {
        const mkIdx = headers.findIndex(
          (h) => h.includes("macedonian") || h.includes("question")
        );
