...
import { X, CheckCircle2, XCircle, Zap, RotateCcw, AlertTriangle } from 'lucide-react';
...
const Mascot = ({ emotion }: { emotion: 'neutral' | 'happy' | 'sad' }) => (
  <div className={`w-14 h-14 bg-white rounded-full border-[3px] border-gray-200 shadow-md flex items-center justify-center text-2xl transition-transform duration-300 ${emotion === 'happy' ? 'scale-110 border-brand rotate-12' : emotion === 'sad' ? 'scale-90 border-danger -rotate-12' : ''}`}>
    {emotion === 'neutral' && '🦁'}
    {emotion === 'happy' && '🤩'}
    {emotion === 'sad' && '🙈'}
  </div>
...
const generateRecapQuestions = (currentModId: string, currentLessonId: string): QuizQuestion[] => {
    ...
    if (lessonIndex > 0) {
        ...
        picked.forEach(q => recaps.push({ ...q, id: `recap_1_${q.id}_${Date.now()}`, isRecap: true }));
    }
    if (lessonIndex > 1) {
        ...
        picked.forEach(q => recaps.push({ ...q, id: `recap_2_${q.id}_${Date.now()}`, isRecap: true }));
    }
    return recaps;
};
...
export const QuizView = () => {
  ...
  const handleResult = (isCorrect: boolean) => {
    if (isCorrect) {
      ...
      confetti({ particleCount: 50, spread: 60, origin: { y: 0.8 }, colors: ['#58cc02', '#89e219'] });
    } else {
      setStatus('wrong');
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-white relative overflow-hidden">
        ...
        <div className="animate-pop text-center z-10 w-full max-w-sm">
            <div className="w-24 h-24 bg-yellow-400 rounded-full mx-auto mb-6 flex items-center justify-center text-5xl shadow-xl border-4 border-yellow-200">🏆</div>
            <h2 className="text-3xl font-black text-brand-dark mb-2 tracking-tight">Lesson Complete!</h2>
            ...
            <Button onClick={() => navigate('/')} fullWidth className="text-lg py-3 shadow-xl">Continue</Button>
        </div>
      </div>
    );
  }

  if (!lesson || !currentQ) return <div className="flex h-screen items-center justify-center font-bold text-gray-400 animate-pulse">Loading...</div>;

  const isConnectType = currentQ.type === 'connect';
  const showCheckButton = !isConnectType;
  const progressPercentage = Math.min(100, (correctCount / totalInitialQuestions) * 100);
  const mistakesCount = queue.length > (totalInitialQuestions - correctCount) ? queue.length - (totalInitialQuestions - correctCount) : 0;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <div className="px-4 py-4 flex items-center gap-4 max-w-md mx-auto w-full z-10 relative bg-slate-50/90 backdrop-blur-sm">
        ...
        <div className="flex-1">
             <div className="w-full h-3 bg-gray-200 rounded-full overflow-hidden relative transform translate-z-0">
                <div className="h-full bg-brand transition-all duration-500 ease-out rounded-full relative" style={{ width: `${progressPercentage}%` }}>
      ...
      <div className="flex-1 px-4 flex flex-col max-w-md mx-auto w-full pb-32 relative">
        ...
        {/* --- QUESTION CONTENT --- */}
        <div className={`relative z-10 ${status === 'wrong' ? 'animate-shake' : ''}`}>
            ...
            {!isConnectType && (
                <div className="absolute -top-4 -right-2 z-20 transform rotate-6">
                     <Mascot emotion={status === 'idle' ? 'neutral' : status === 'correct' ? 'happy' : 'sad'} />
                </div>
            )}
      ...
      `}>
        <div className="max-w-md mx-auto flex justify-between items-center gap-4">
            <div className="flex-1">
                {status === 'idle' && !isConnectType && <div className="hidden sm:block text-gray-400 font-bold text-xs uppercase tracking-wider pl-1">Select answer</div>}
                {status === 'correct' && (
                    <div className="flex items-center gap-2 text-brand-dark font-extrabold text-lg animate-pop">
                        <div className="bg-white p-1 rounded-full shadow-sm"><CheckCircle2 className="w-6 h-6 fill-brand text-white" /></div>
                        <div>Excellent!</div>
                    </div>
                )}
                {status === 'wrong' && (
                    <div className="animate-shake">
                        <div className="flex items-center gap-2 text-danger-dark font-extrabold text-lg mb-0.5">
                            <XCircle className="w-6 h-6 fill-danger text-white" /> Incorrect
                        </div>
                        <div className="text-danger-dark text-xs font-bold ml-8 opacity-80">Correct: {currentQ.correctAnswer || 'Match pairs'}</div>
                    </div>
                )}
            </div>

            {status === 'correct' || status === 'wrong' ? (
                <Button onClick={next} variant={status === 'correct' ? 'primary' : 'danger'} className="w-full sm:w-auto min-w-[140px] text-lg shadow-lg py-3">CONTINUE</Button>
            ) : (
                showCheckButton ? (
                    <Button onClick={check} disabled={!sel} variant="secondary" className="w-full sm:w-auto min-w-[140px] text-lg shadow-lg py-3">CHECK</Button>
                ) : (
                    <div className="text-gray-400 font-bold text-xs uppercase tracking-widest opacity-50 mr-2">Match all pairs</div>
                )
            )}
        </div>
      </div>
