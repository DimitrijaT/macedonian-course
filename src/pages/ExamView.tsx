...
export const ExamView = () => {
  ...
  if (isFinished) {
    ...
    return (
      <div
        className={`min-h-screen flex flex-col items-center justify-center p-6 ${passed ? "bg-green-50" : "bg-red-50"}`}
      >
        <div className="bg-white p-8 rounded-3xl shadow-2xl text-center max-w-sm w-full border-4 border-white ring-4 ring-black/5 animate-pop">
          {passed ? (
            <>
              ...
              <div className="text-5xl font-black text-brand mb-4">
                {percentage}%
              </div>
              <p className="text-gray-400 font-bold mb-8 uppercase tracking-wide">
                Mastery Unlocked
              </p>
            </>
          ) : (
    ...
    <div className="min-h-screen flex flex-col bg-slate-50 overflow-hidden">
      ...
      >
        <div className="max-w-2xl mx-auto flex justify-between items-center gap-4">
          <div className="flex-1">
            {status === "correct" && (
              <div className="text-brand-dark font-extrabold text-xl animate-pop">
                Correct!
              </div>
            )}
            {status === "wrong" && (
              <div className="animate-shake">
                <div className="text-danger-dark font-extrabold text-xl">
                  Incorrect
                </div>
                <div className="text-danger-dark text-xs font-bold">
                  Ans: {q.correctAnswer}
                </div>
              </div>
            )}
          ...
          {status === "correct" || status === "wrong" ? (
            <Button
              ...
              className="w-auto min-w-[150px] text-lg shadow-lg"
            >
              CONTINUE
            </Button>
          ) : showCheckButton ? (
            ...
            <div className="text-gray-400 font-bold text-sm uppercase tracking-widest opacity-50 mr-4">
              Match all pairs
            </div>
          )}
        </div>
